...
function breadcrumb_trail( $args = array() ) {
	...
	$ecommerce_clothing_breadcrumb = apply_filters( 'breadcrumb_trail_object', null, $args );

	if ( ! is_object( $ecommerce_clothing_breadcrumb ) ) {
		$ecommerce_clothing_breadcrumb = new Breadcrumb_Trail( $args );
	}
...
class Breadcrumb_Trail {
	...
	public function __construct( $args = array() ) {
		...
		$this->args = apply_filters( 'breadcrumb_trail_args', wp_parse_args( $args, $ecommerce_clothing_defaults ) );
	...
	public function trail() {
		...
		if ( 0 < $ecommerce_clothing_item_count ) {
			...
			foreach ( $this->items as $item ) {
				...
				preg_match( '/(<a.*?>)(.*?)(<\/a>)/i', $item, $ecommerce_clothing_matches );

				// Wrap the item text with appropriate itemprop.
				$item = ! empty( $ecommerce_clothing_matches ) ? sprintf( '%s<span itemprop="name">%s</span>%s', $ecommerce_clothing_matches[1], $ecommerce_clothing_matches[2], $ecommerce_clothing_matches[3] ) : sprintf( '<span itemprop="name">%s</span>', $item );

				// Wrap the item with its itemprop.
				$item = ! empty( $ecommerce_clothing_matches )
				...
				$meta = sprintf( '<meta itemprop="position" content="%s" />', absint( $ecommerce_clothing_item_position ) );

				// Build the list item.
				$ecommerce_clothing_breadcrumb .= sprintf( '<%1$s %2$s>%3$s%4$s</%1$s>', tag_escape( $this->args['item_tag'] ), $attributes, $item, $meta );
			}
	...
	protected function add_term_archive_items() {
		...
		if ( false !== $taxonomy->rewrite ) {
			...
			if ( $taxonomy->rewrite['slug'] ) {
				...
				// type archive.
				$ecommerce_clothing_matches = explode( '/', $slug );
				...
				if ( isset( $ecommerce_clothing_matches ) ) {
					...
					foreach ( $ecommerce_clothing_matches as $match ) {

						// If a match is found.
						$slug = $match;

						// Get public post types that match the rewrite slug.
						$ecommerce_clothing_post_types = $this->get_post_types_by_slug( $match );
		...
		if ( false === $done_post_type && 1 === count( $taxonomy->object_type ) && post_type_exists( $taxonomy->object_type[0] ) ) {
			...
			if ( 'post' === $taxonomy->object_type[0] ) {
				...
				$ecommerce_clothing_post_type_object = get_post_type_object( $taxonomy->object_type[0] );
	...
	protected function add_day_archive_items() {
		...
		$year  = sprintf( $this->labels['archive_year'], get_the_time( esc_html_x( 'Y', 'yearly archives date format', 'ecommerce-clothing' ) ) );
		$month = sprintf( $this->labels['archive_month'], get_the_time( esc_html_x( 'F', 'monthly archives date format', 'ecommerce-clothing' ) ) );
		$day   = sprintf( $this->labels['archive_day'], get_the_time( esc_html_x( 'j', 'daily archives date format', 'ecommerce-clothing' ) ) );
	...
	protected function add_week_archive_items() {
		...
		$year = sprintf( $this->labels['archive_year'], get_the_time( esc_html_x( 'Y', 'yearly archives date format', 'ecommerce-clothing' ) ) );
		$week = sprintf( $this->labels['archive_week'], get_the_time( esc_html_x( 'W', 'weekly archives date format', 'ecommerce-clothing' ) ) );
	...
	protected function add_post_parents( $ecommerce_clothing_post_id ) {
		...
		while ( $ecommerce_clothing_post_id ) {
			...
			$parents[] = sprintf( '<a href="%s">%s</a>', esc_url( get_permalink( $ecommerce_clothing_post_id ) ), get_the_title( $ecommerce_clothing_post_id ) );
	...
	protected function get_post_types_by_slug( $slug ) {
		...
		foreach ( $ecommerce_clothing_post_types as $type ) {
	 ...
	 * @return void
	 */
	protected function add_post_terms( $ecommerce_clothing_post_id, $taxonomy ) {

		// Get the post type.
		$ecommerce_clothing_post_type = get_post_type( $ecommerce_clothing_post_id );

		// Get the post categories.
		$terms = get_the_terms( $ecommerce_clothing_post_id, $taxonomy );
			...
			if ( isset( $ecommerce_clothing_matches ) ) {
				...
				foreach ( $ecommerce_clothing_matches as $match ) {
					...
					if ( isset( $match[0] ) ) {
						...
						$ecommerce_clothing_path = str_replace( $match[0], '', $ecommerce_clothing_path );
		...
		if ( is_array( $ecommerce_clothing_matches ) ) {
			...
			foreach ( $ecommerce_clothing_matches as $match ) {
				...
				elseif ( taxonomy_exists( trim( $tag, '%' ) ) ) {
					...
					$this->add_post_terms( $ecommerce_clothing_post_id, trim( $tag, '%' ) );
				}
			}
		}
	}
